...
function ListMoves(startMove) {
  let list = {
    ...
    getAllMoves: function (move, mainMove, rawNotation) {
      let res =
        mainMove == null || move !== mainMove ? (rawNotation ? this.displayRawMove(move) : this.displayMove(move)) : '';
      if (move.mainLine)
        res +=
          move.next_moves.length === 0
            ? ''
            : rawNotation
            ? this.displayRawMove(move.next_moves[0])
            : this.displayMove(move.next_moves[0]);
      else
        res +=
          move.next_moves.length === 0
            ? ') '
      ...
      if (move.next_moves.length > 0) res += this.getAllMoves(move.next_moves[0], move.next_moves[0], rawNotation);
      return res;
    },
...
function parseToMove(movesArray, prevMove, arrayIndex, chess) {
  if (arrayIndex === movesArray.length - 1) return;
