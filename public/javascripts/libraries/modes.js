function gameMode(chessGame, isAllowOnlyOneSideMode, noOverMode) {
  ...
  let onDragStart = function (source, piece, position, orientation) {
    if (
      (chessGame.chess.game_over() === true && !noOverMode) ||
      (chessGame.chess.turn() === 'w' && piece.search(/^b/) !== -1) ||
      (chessGame.chess.turn() === 'b' && piece.search(/^w/) !== -1) ||
      (isAllowOnlyOneSideMode && chessGame.chess.turn() !== color)
    ) {
      return false;
    }
  };
...
function engineMode(chessGame) {
  ...
  let onDragStart = function (source, piece, position, orientation) {
    if (chessGame.chess.in_checkmate() === true || chessGame.chess.in_draw() === true || piece.search(/^b/) !== -1) {
      return false;
    }
  };
  engine.onmessage = function (event) {
    let match = event.match(/^bestmove ([a-h][1-8])([a-h][1-8])([qrbk])?/);
    if (match) chessGame.chess.move({ from: match[1], to: match[2], promotion: match[3] });
    chessGame.board.position(chessGame.chess.fen());
  };
...
function puzzleMode(chessGame) {
  ...
  let onDragStart = function (source, piece, position, orientation) {
    if (
      chessGame.chess.game_over() === true ||
      (chessGame.chess.turn() === 'w' && piece.search(/^b/) !== -1) ||
      (chessGame.chess.turn() === 'b' && piece.search(/^w/) !== -1) ||
...
let promotionDialog = function (chessGame, callback) {
  function createPieceElem(piece, color) {
    let $elem = $(
      '<div class="p-2  promotionPiece"><img class="align-middle img-thumbnail " src="images/chesspieces/wikipedia/' +
        color +
        piece.toUpperCase() +
        '.png"/></div>'
    );
  ...
  let elem = $('<div class="promotionDialog container">' + '</div>');
  elem.append($flex);
  elem.width(width);
  elem.height(width);
