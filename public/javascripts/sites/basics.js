...
function checkHandler(isCheckingSide) {
  return {
    ...
    checkPositionGenerator: function (chess, isCheckingSide) {
      chess.clear();
      if (!isCheckingSide) PositionManipulator.changeSideOnMove(chess);
...
let positionHandler = {
  ...
  update: function (chessBoard, move) {
    ...
    let handler = this;
    timeout = setTimeout(function () {
      handler.init(chessBoard);
    }, 500);
  },
  validator: null,
  validate: function (chess, move) {
    if (this.validator !== null) counters.increaseCounter(this.validator(chess, move));
  },
};
  ...
  let eventHandler = {
    ...
    update: function (chessBoard, move) {
      ...
      if (numberOfPawns === 0) {
        ...
        if (this.levelAccepted && this.level < 7) this.level++;
