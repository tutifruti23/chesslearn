...
let isGameOver = function (lastPlayerMove) {
  if (chessGame.chess.game_over() && !settings.reviewMode) {
    let score;
    if (chessGame.chess.in_draw()) score = 0;
    else {
...
function exerciseCompleted(result, callback) {
  ...
  switch (result) {
    case 1:
      ...
      break;
    case 0:
      exerciseComplete = userAndPuzzleData.exerciseData.result === 'draw';
      break;
    case -1:
...
let exerciseHandler = {
  ...
  update: function (chessBoard, move) {
    ...
    if (!isGameOver(true)) {
      setTimeout(function () {
        engine.goDepth(chessBoard.chess.fen(), 15);
      }, 50);
    }
  },
};
...
let userAndPuzzleData = new Vue({
  ...
  methods: {
    ...
    getUserLastAttemptDate: function () {
      return this.exerciseData === undefined || this.exerciseData.lastTimeSolved === undefined
        ? '?'
        : this.exerciseData.lastTimeSolved.slice(0, 10);
    },
    getUserAttempts: function () {
      return this.exerciseData === undefined || this.exerciseData.attempts === undefined
        ? '?'
        : this.exerciseData.attempts;
    },
