...
let puzzleSpareHandler = {
  ...
  update: function (chessGame, chessboardFen) {
    chessGame.chess.load(
      PositionManipulator.chessboardFenToFen(
        chessboardFen,
        settings.getColor(),
        settings.castling(),
        settings.getEnpassant()
      )
    );
  },
};
let settings = new Vue({
  ...
  methods: {
    castling: function () {
      let res = '';
      if (this.wkCastle === true) res += 'K';
      if (this.wqCastle === true) res += 'Q';
      if (this.bkCastle === true) res += 'k';
      if (this.bqCastle === true) res += 'q';
      return '-';
    },
    ...
    setEngineLine: function (num, line, cp, mate) {
      ...
      switch (num) {
        case 1:
          this.firstEngineLine = '1. ' + notation;
          break;
        case 2:
          this.secondEngineLine = '2. ' + notation;
          break;
        case 3:
          this.thirdEngineLine = '3. ' + notation;
          break;
      }
    },
    ...
    saveExercise: function () {
      ...
      $.post(
        ...
        function (isSaved) {
          if (isSaved) addSuccessInfo('Saved!');
          else addErrorInfo('Error');
        }
      );
    },
    setPosition(pos) {
      switch (pos) {
        case 'start':
          chessGame.setPosition(PositionManipulator.getStartFen());
          this.setCastles(true);
          break;
        case 'empty':
          chessGame.setPosition(PositionManipulator.getEmptyBoardFen());
          this.setCastles(false);
          break;
      }
    },
    updateFen: function () {
      ...
      chessGame.chess.load(
        PositionManipulator.chessboardFenToFen(chessboardFen, this.getColor(), this.castling(), '-')
      );
    },
